...
from engines.setup_engine import classify_setup
...
def build_summary_output(
    ...
    text += f"Trade Quality: {scores['quality']}\n\n"
    ...
    text += "\nLiquidity Zones:\n"
    text += f"Resistance Zone: {sr_analysis['resistance_zone']}\n"
    text += f"Support Zone: {sr_analysis['support_zone']}\n"
    ...
    text += "\nSetup Classification:\n"
    text += f"{setup_analysis['setup']}\n"
    text += f"Confidence: {setup_analysis['confidence']}\n"
    text += f"Probability: {setup_analysis['probability']}\n"
